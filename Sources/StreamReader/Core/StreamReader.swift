...
public protocol StreamReader : AnyObject {
	...
	/**
	 ...
	 Can be greater than `readSizeLimit` iif `readSizeLimit` has been changed to a lower value than `currentStreamReadPosition`. */
	var currentStreamReadPosition: Int {get}
	
	/**
	 Force set `streamHasReachedEOF` to `false`.
	 Call this before calling the read methods to force a read when EOF has been reached. */
	func clearStreamHasReachedEOF()
	
	/**
	 The index of the first byte returned from the stream at the next read, where 0 is the first byte of the stream.
	 
	 This is also the number of bytes that has been returned by the different read methods of the stream, when `updateReadPosition` was `true`. */
	var currentReadPosition: Int {get}
	
	/**
	 The maximum value possible for `currentReadPosition` **and** the maximum total number of bytes allowed to be read _from the underlying stream_.
	...
	/**
	 ...
	 Reading a data of size 0 must never fail and always return an empty Data object.
	 
	 - Important: For the memory to stay valid and immutable in the handler, do **NOT** do any stream operation inside the handler.
	 
	 - Parameter size: The size you want to read from the buffer.
	 - Parameter allowReadingLess: If end of stream (or read size limit) is reached before the required size is read, should we return what can be read?
	 - Parameter updateReadPosition: If `true`, the `currentReadPosition` will be updated; if `false` it won’t.
	 - Parameter handler: Use the data inside this hanlder.
	 Do **NOT** do any stream operation inside the handler.
	 - Parameter bytes: A raw buffer pointer to the bytes that have been read.
	 - Throws: If any error occurs reading the data (including end of stream reached before the given size is read if `allowReadingLess` is `false`), an error is thrown.
	...
	/**
	 Read from the stream, until one of the given delimiters is found.
	 ...
	 Choose your matching mode with care.
	 ...
	 - Parameter delimiters: The delimiters you want to stop reading at.
	 If this array is empty, the stream is read to the end.
	 - Parameter matchingMode: How to choose which delimiter will stop the reading of the data.
	 - Parameter failIfNotFound: If none of the delimiters are found, should the method throw or return the data up to the end of the stream?
	 - Parameter includeDelimiter: Should the returned data include the delimiter that matched?
	 - Parameter updateReadPosition: If `true`, the `currentReadPosition` will be updated; if `false` it won’t.
	 - Parameter handler: Use the data inside this hanlder.
	 Do **NOT** do any stream operation inside the handler.
	 - Parameter bytes: A raw buffer pointer to the bytes that have been read.
	 - Parameter delimiterThatMatched: The delimiter that matched to stop reading the stream.
	 If no delimiters have been given (read the stream to the end), this parameter will contain an empty Data object.
...
public extension StreamReader {
	...
	/**
	 Returns `true` if the underlying stream has reached EOF _and_ the current stream read position is the same (or lower than) the current read position. */
	...
	/**
	 Reads a line from the stream, supporting unix, legacy MacOS and windows new lines.
	 By default only unix new lines are considered.
	 
	 The line is returned as a `Data` object, which you can then use to init a `String` using the encoding you wish.
	 
	 The line separator found is also returned.
	 If empty, it means the end of the stream has been reached before a line separator could be found.
	 
	 - Important: The newline is not returned in the line, however the stream position (next read) is set _after_ the line separator.
	 
	 - Returns: The line and line separator, or `nil` if the end of the stream was reached. */
	func readLine(allowUnixNewLines: Bool = true, allowLegacyMacOSNewLines: Bool = false, allowWindowsNewLines: Bool = false) throws -> (line: Data, newLineChars: Data)? {
		...
		if !allowWindowsNewLines || !allowLegacyMacOSNewLines || separator == lf || separator.isEmpty {
			...
			/* Just a reminder of the if above.
			 * Asserted, because we use this truth in the logic of the code that follow. */
			assert(allowWindowsNewLines && allowLegacyMacOSNewLines)
			
			/* All we have to do is check the next char in the stream.
			 * If it is an lf, we have a windows line separator, otherwise we have a legacy MacOS separator. */
...
/**
 ...
 - We'll use a `GenericStreamReader`, which uses a cache to hold some of the data read from the stream;
...
public enum DelimiterMatchingMode {
	...
	/**
	 ...
	 However, any delimiter can match, the implementation is really up to the implementer…
	 Implementers should keep in mind the goal of this matching mode is to match and return the data in the quickest way possible. */
	case anyMatchWins
	
	/**
	 The matching delimiter that gives the shortest data will be used.
	 In our example, it will be the fourth one (`"12345"`) which will yield the shortest data (`"0"`). */
	case shortestDataWins
	
	/**
	 The matching delimiter that gives the longest data will be used.
	 In our example, it will be the second one (`"67"`) which will yield the longest data (`"012345"`).
	 
	 - Important: Use this matching mode with care!
	 It might have to read all of the stream (and thus fill the memory with it) to be able to correctly determine which match yields the longest data.
	 Actually, the only case where the result can be returned safely before reaching the end of the data is when all of the delimiters match… */
	case longestDataWins
	
	/**
	 The first matching delimiter will be used.
	 In our example, it will be the first one (`"45"`).
