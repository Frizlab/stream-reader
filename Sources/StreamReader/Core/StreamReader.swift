...
public protocol StreamReader : AnyObject {
	...
	/**
	 Force set `streamHasReachedEOF` to `false`. Call this before calling the
	 read methods to force a read when EOF has been reached. */
	...
	/**
	 ...
	 This is also the number of bytes that has been returned by the different
	 read methods of the stream, when `updateReadPosition` was `true`. */
	...
	/**
	 ...
	 back to a stream once it has been read from. If you know your stream can
	 have more data that what you’re interested in and want to avoid the reader
	 reading too much and thus rendering the stream unusable for other clients,
	...
	/**
	 ...
	 (or the stream size limit) and the returned `delimiterThatMatched` will be
	 ...
	 **NOT** do any stream operation inside the handler.
	 ...
	 - Parameter matchingMode: How to choose which delimiter will stop the
	 reading of the data.
...
public extension StreamReader {
	...
	/**
	 ...
	 - Important: If the read size limit has not been reached and EOF has not
	 been definitely reached, one byte might be read from the underlying stream
	 when using this method, though the `currentReadPosition` of the stream
	 reader will never change. */
	...
	/**
	 ...
	 - Returns: The line and line separator, or `nil` if the end of the stream
	 was reached. */
...
/**
 ...
 - We'll use a `GenericStreamReader`, which uses a cache to hold some of the
...
public enum DelimiterMatchingMode {
	...
	/**
	 ...
	 example, it will be the second one (`"67"`) which will yield the longest
	 data (`"012345"`).
	 ...
	 the result can be returned safely before reaching the end of the data is
	 when all of the delimiters match… */
	...
	/**
	 ...
	 `shortestMatchingDelimiterWins` you can do it yourself by using this
	 matching mode and simply sorting your delimiters list before giving it to
	 the function.*/
